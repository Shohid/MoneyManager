...
public class Tab1  extends Fragment{
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        Cursor cursor1=sqLiteDatabase.rawQuery( "SELECT  strftime('%Y-%m',dateyo) as month_of_year, sum(amount) as total FROM student_fooding_expanse_view_p GROUP BY month_of_year",null);
        int i=0;
      ...
      final String[]symbol = {"","৳","₹","₱","£","kr","$","$","Дин.","RM","Rf."};
        ...
        sqLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS student_fooding_expanse_view_p(id INTEGER PRIMARY KEY AUTOINCREMENT,amount ,place VARCHAR,dateyo VARCHAR,timeyo VARCHAR,viewid INTEGER)");//create table
        Cursor cursor_yo = sqLiteDatabase.rawQuery("SELECT SUM(amount) FROM student_fooding_expanse_view_p", null);
        try {
      ...
      Cursor cursor2=sqLiteDatabase.rawQuery( "SELECT  strftime('%d-%m',dateyo) as date, sum(amount) as total1 FROM student_fooding_expanse_view_p  GROUP BY date order by date DESC limit 8 ",null);
        int j=0;
